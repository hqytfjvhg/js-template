        ...
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item
              ...
              :icon="RefreshLeft"
              @click="pageReload"
              >{{ $t("message.system.tab.reload") }}</el-dropdown-item
            >
            <el-dropdown-item
              class="tab-ddropdown-item"
              :icon="CircleClose"
...
//重新加载或刷新
    ...
    function setPosition() {
      if (scrollbarDom.value) {
        ...
        //使用getBoundingClientRect()方法获取上述三个关键DOM元素的边界信息（宽度、高度及相对于视口的位置
        const domData = {
    ...
    function handleWhellScroll(e) {
      ...
      // console.log(scrollLeft.value + eventDelta / install$4,"鼠标滚动")
      scrollbarDom.value?.setScrollLeft(scrollLeft.value + distance * speed)
    }

    /** 监听滚动事件,元素小于页面宽度，不显示滚动条，无法触发 */
